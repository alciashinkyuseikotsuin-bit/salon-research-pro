...
## バックログ（優先順）
1. [ ] 生成待ちの経過秒数表示（台本/LINE/コピー。「壊れた？」と思わせない）
2. [ ] 自店プロフィール登録（業種・店名・地域・強み・実績数字をlocalStorage保存→全生成APIに salon_profile として注入。固有名詞と数字で成果物の精度UP）
3. [ ] エラー時のUX（alert廃止→画面内メッセージ＋「もう一度試す」ボタン）
4. [ ] 再生成の世代比較（前回の結果を残して並べる・採用ボタン)
5. [ ] 初回ミニチュートリアル（初アクセス時に「まず🔍から3分で体験」）
6. [ ] 当たりコピーのフィードバック記録（使った結果を記録→勝ちスワイプとして生成プロンプトに還元)
7. [ ] リサーチ源の強化（Googleサジェスト追加・知恵袋依存の低減）
8. [ ] ロープレ音声対応（Web Speech APIで音声入力＋お客様AIの読み上げ）
